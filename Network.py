...
def Evaluate(self,indata, decision_plotting=0): # evaluates all layers
    ...
    for i in range(1,len(self.numLayers)):
        self.Layers[i].Evaluate(self.Layers[i-1].a) #if decision_plotting == 0:
        return self.Layers[len(self.numLayers)-1].a #else:
# if self.Layers[len(self.numLayers)-1].a[0] > 0.5: # return 1
# else:
# return 0
    ...
    for j in range(epochs):
        ...
        for i in range(self.X.shape[0]):
                ...
                if (self.lastLayerAF == ActivationType.SOFTMAX):
                    error += -(self.Y[i] * np.log(self.Layers[len(self.numLayers)-
1].a+0.001)).sum()
                else:
                    error += ((self.Layers[len(self.numLayers)-1].a - self.Y[i]) * \
                              (self.Layers[len(self.numLayers)-1].a -
self.Y[i])).sum()
                ...
                while(lnum >= 0):
                    if (lnum == len(self.numLayers)-1): # last layer
                        if (self.lastLayerAF == ActivationType.SOFTMAX): self.Layers[lnum].delta = -self.Y[i]+ self.Layers[lnum].a
                            ...
                            self.Layers[lnum].delta = -(self.Y[i]-
self.Layers[lnum].a) * self.Layers[lnum].derivAF
                    else: self.Layers[lnum].delta = np.dot(self.Layers[lnum+1].W.T,self.Layers[lnum+1].delta) * \
...
def UpdateGradsBiases(self, learningRate, lambda1, batchSize): # update weights and biases for all layers
