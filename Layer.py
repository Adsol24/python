...
class Layer(object):
    ...
    0.2,activationType=ActivationType.SIGMOID):
        # initialize the weights and biases self.numNeurons = numNeurons
        ...
        self.derivAF = np.zeros((numNeurons,1)) # deriv of Activation function #self.W = 0.01 * np.random.randn(numNeurons,numNeuronsPrevLayer) #self.W = np.random.uniform(low=-
        #0.1,high=0.1,size=(numNeurons,numNeuronsPrevLayer))
#self.b = np.random.uniform(low=-1,high=1,size=(numNeurons,1))
    ...
    def Evaluate(self,indata):
        ...
        if (self.activationFunction == ActivationType.SOFTMAX):
            ...
            self.derivAF = None # we do delta computation in Softmax layer if (self.lastLayer == False):
